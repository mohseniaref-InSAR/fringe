...
def getLinePixelBbox(geobbox, latFile, lonFile):
    ...
    ymin = np.int32(np.round(np.min([se[1], nw[1]])))
    ymax = np.int32(np.round(np.max([se[1], nw[1]])))

    xmin = np.int32(np.round(np.min([se[0], nw[0]])))
    xmax = np.int32(np.round(np.max([se[0], nw[0]])))
